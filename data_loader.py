...
def get_efficient_dataloaders(width_height=192, num_points=1, batch_size=1, input_directory='torch_inputs/', label_directory='/'):
	
	total_images = len(os.listdir(input_directory))
	# print('Total points', total_images)

	# def load_from(directory, prefix):
	# 	tensors = []
	# 	for tensor in os.listdir(directory):
	# 		t = torch.load(directory)
	# 		tensors.append(t)


	class SegmentDataset(Dataset):
		def __init__(self, input_directory, label_directory):
		...
		def __getitem__(self, index):
			image_name = self.input_directory + 'input_' + str(index) + '.pt'
			label_name = self.label_directory + 'label_' + str(index) + '.pt'
	...
	train_set = SegmentDataset(input_directory, label_directory)
	val_set = SegmentDataset(input_directory, label_directory)
...
def get_PT_dataloaders(width_height=192, num_points=1, batch_size=1, input_directory='inputs_overfit/', label_directory='labels_overfit/'):
	...
	labels = list(zip(*datasets.ImageFolder(root=label_directory, transform=preprocess_transform)))[0][:num_points]
	...
	train_set = SegmentDataset(train_images, train_labels, random_flip=False)
	# train_set = SegmentDataset(inputs[-NUM_EVAL_IMAGES:], labels[-NUM_EVAL_IMAGES:], random_flip=True)
	val_set = SegmentDataset(val_images, val_labels, random_flip=False)
