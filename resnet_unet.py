...
def convrelu(in_channels, out_channels, kernel_size, padding, activation=nn.ReLU, pooling=None):
	if pooling is None:
...
class ResNetUNet(nn.Module):
	...
	def __init__(self, dict_args):
		...
		activation = dict_args['activation']

		# print('NUMCLASSES', num_classes)
		# print('ACTVATION', activation)

		self.base_model = models.resnet18(pretrained=True)
		self.base_layers = list(self.base_model.children())

		self.layer0 = nn.Sequential(*self.base_layers[:3]) # size=(N, 64, x.H/2 x.W/2)TODO:?
		self.layer0_1x1 = convrelu(64, 64, 1, 0, activation)
		self.layer1 = nn.Sequential(*self.base_layers[3:5]) # size=(N, 64, x.H/4, x.W/4)
		self.layer1_1x1 = convrelu(64, 64, 1, 0, activation)
		self.layer2 = self.base_layers[5] # size=(N, 128, x.H/8, x.W/8)
		self.layer2_1x1 = convrelu(128, 128, 1, 0, activation)
		self.layer3 = self.base_layers[6] # size=(N, 256, x.H/16, x.W/16)
		self.layer3_1x1 = convrelu(256, 256, 1, 0, activation)
		self.layer4 = self.base_layers[7] # size=(N, 512, x.H/32, x.W/32)
		self.layer4_1x1 = convrelu(512, 512, 1, 0, activation)

		self.upsample = nn.Upsample(scale_factor=2, mode='bilinear', align_corners=True)

		self.conv_up3 = convrelu(256 + 512, 512, 3, 1, activation)
		self.conv_up2 = convrelu(128 + 512, 256, 3, 1, activation)
		self.conv_up1 = convrelu(64 + 256, 256, 3, 1, activation)
		self.conv_up0 = convrelu(64 + 256, 128, 3, 1, activation)

		self.conv_original_size0 = convrelu(3, 64, 3, 1, activation)
		self.conv_original_size1 = convrelu(64, 64, 3, 1, activation)
		self.conv_original_size2 = convrelu(64 + 128, 64, 3, 1, activation)
	...
	def forward(self, input):
		...
		out = self.conv_last(x) # N, num_classes, H, W
...
class ConvNet(nn.Module):
    ...
    def __init__(self, dict_args):
        ...
        args = dict_args.keys()
        dim = dict_args['input_dim']
        num_classes = dict_args['num_classes']
        num_layers = dict_args['num_layers']
        activation = dict_args['activation']
        ...
        for i in range(num_layers):
        	layer = convrelu(in_channels, out_channels, kernel_size=3, padding=0, activation=activation, pooling=pooling)
        	layers.append(layer)
        	in_channels, out_channels = out_channels, out_channels * 2
        	out_dim //= 2


        # self.fc1 = nn.Linear(in_channels * out_dim * out_dim, 120)  # 6*6 from image dimension
        self.fc1 = nn.Linear(in_channels * out_dim * out_dim, 120)  # 6*6 from image dimension
        self.fc2 = nn.Linear(120, 84)
        self.fc3 = nn.Linear(84, num_classes)

        layers.extend([self.fc1, self.fc2, self.fc3])
...
def train_model(model, optimizer, scheduler, num_epochs, dataloaders, device, loss_func=calc_loss):
        ...
        for phase in ['train', 'val']:
            ...
            for inputs, labels in dataloaders[phase]:
                ...
                with torch.set_grad_enabled(phase == 'train'):
                    ...
                    loss = loss_func(outputs, labels, metrics)
            ...
            if phase == 'val' and epoch_loss < best_loss:
                ...
                best_model_wts = copy.deepcopy(model.state_dict())
                # model.save()
                # torch.save(model.state_dict(), 'ckpt_31_all.pth')
